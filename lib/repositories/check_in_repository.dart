...
class CheckInRepository {
  ...
  Future<List<DailyCheckInModel>?> fetchAllCheckInDetails(String userId, {int limit = 7}) async {
    QuerySnapshot<Map<String, dynamic>>? documentData = await FirebaseFirestore.instance.collection('users')
    .doc(userId).collection('daily-check-in').orderBy("date", descending: true).get();
